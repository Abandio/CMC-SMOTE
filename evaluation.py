...
def clustering(x_list, y):
    ...
    if np.min(y) == 1:
        y = y - 1
    accuracy,nmi,ari,pur, confusion_matrix = clustering_metric(y, kmeans_assignments, n_clusters)


    return accuracy,nmi,ari,pur, confusion_matrix #, y_preds.astype('int64')
...
def classification_metric(y_true, y_pred, average='macro', verbose=True, decimals=4):
    ...
    # return {'ACC': accuracy, 'precision': precision, 'recall': recall, 'f_measure': f_score}, confusion_matrix
...
def clustering_metric(y_true, y_pred, n_clusters, verbose=True, decimals=4):
    ...
    accuracy, confusion_matrix = classification_metric(y_true, y_pred_ajusted)
    # classification_metrics = classification_metric(y_true, y_pred_ajusted)
    ...
    return accuracy,nmi,ari,pur, confusion_matrix
